...
let getname = localStorage.getItem('dides')
getname = JSON.parse(getname)

let checkname = getname.map(n => n.name)
profileInfoName.textContent = checkname
...
function getItem() {
    let getMessages = localStorage.getItem('clientMessage')
...
function setItem() {
    localStorage.setItem('clientMessage', JSON.stringify(friendsM))
}
...
function send(checkname,typing){
    friendsM.push({
            name: `${checkname}`,
            message: typing,
            age: 34
